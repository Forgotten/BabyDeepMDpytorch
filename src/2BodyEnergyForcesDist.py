...
from networks import DeepMDsimpleEnergyForces
...
potentialArrayTorch = torch.tensor(potentialArray, 
                                   dtype = torch.float32)
forcesArrayTorch = torch.tensor(forcesArray, 
                                   dtype = torch.float32)
...
# building the model 
model = DeepMDsimpleEnergyForces(Npoints, lengthCell, maxNumNeighs,
                           filterNet, fittingNet, False, # no resnet for now
                           ave, std).to(device)

# model = torch.jit.script(DeepMDsimpleEnergyForces(Npoints, lengthCell, maxNumNeighs,
#                            filterNet, fittingNet, True, # no resnet for now
#                            ave, std)).to(device)
...
for epoch in range(1, Nepochs+1):
    ...
    # train the model #
    ###################
    for pos, energy, forces in dataloaderTrain:
        ...
        (energyNN, forcesNN) = model(pos, neighbor_list)
